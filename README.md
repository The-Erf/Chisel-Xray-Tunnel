...
## 🧐 تانل Chisel برای کدوم پروتکل ها کاربردیه؟   
...
```
  توی فایل سرویس بالا هم بجای  Yourdomain.com دامنه خودتون رو قرار بدید، اون پورت 8000 هم پورت listen چیزل هست و باید یه پورت جدید وارد کنید(برای ترکیب کردن با سایر روش ها مثل تانل اوتباند به این پورت نیاز دارید) و 443 هم همونطور که مشخصه پورت کانفیگ سرور خارجمون هستش که اگر پورت کانفیگتون فرق میکنه باید از اینجا هم تغییرش بدید😊     
...
اگه در حال حاضر سرور ریلیتی دارید و با اختلالات جدید به مشکل خوردید و مجبورید سرورتون رو تانل کنید،متود فورواردر Chisel میتونه کمکتون کنه :)
به این صورت که شما سرور خارجتون رو میبرید پشت cdn کلادفلر(پروکسی رو روشن میکنید) و این تانل رو روی سرور ایران برقرار میکنید،حالا فقط باید در قسمت Address کانفیگتون،ایپی یا دامنه سرور ایرانتون رو قرار بدید که اگه از دامنه استفاده میکنید چه بهتر! فقط کافیه از پشت دامنه ip سرور ایران رو جایگزین ip سرور خارجتون کنید و تمام:) یوزراتون بدون تغییری در کانفیگ ها با سرعت قابل قبولی متصل میشن و دیگه اهمیت نداره ایپی سرور خارجتون تمیزه یا نه😅
...
## 😭 رفع مشکلات احتمالی  
 ...
 اگه اینستاگرام درست لود نشد یا اصلا لود نشد احتمالا باید مشکل از dns باشه،تیکه کد زیر رو بالای قسمت inbound قرار بدید تا مشکلتون رفع شه  
...
اگه در متود Site-to-Site اتصال برقرار بود اما بهتون پینگ نمیداد یا پینگش استیبل نبود احتمالا علتش تنظیمات Mux هستش که  با فعال و تنظیم کردنش مطابق کد زیر مشکل رفع میشه
